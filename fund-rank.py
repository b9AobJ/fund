...
def main(argv):
	...
	for fund in all_funds_list:
		print 'process fund:\t' + fund[0].encode('utf-8') + '\t' + fund[2].encode('utf-8')
		strfundcode = fund[0]
		# 获取净值
		jingzhimin = get_jingzhi(strfundcode, strsdate)
		jingzhimax = get_jingzhi(strfundcode, stredate)
	...
	for index in range(len(all_funds_list)):
		...
		strcontent = str(index + 1) + '\t' + all_funds_list[index][0].encode('utf-8') + '\t' + all_funds_list[index][
			2].encode('utf-8') + \
					 '\t\t' + all_funds_list[index][3].encode('utf-8') + '\t\t' + all_funds_list[index][5].encode(
			'utf-8') + '\t\t' + \
					 all_funds_list[index][6].encode('utf-8') + '\t\t' + str(all_funds_list[index][7]) + '\t' + str(
	...
	# 6 排序 写文件 打印结果
	codeFileobject = open('result_' + strsdate + '_' + stredate + '_code.txt', 'w')
	...
	# 		  strsdate + '\t' + stredate + '\t' + '������' + '\t' + '������' + '\n'
	# print strhead
	# codeFileobject.write(strhead)

	# 打印
	for index in range(len(all_funds_list)):
		# print all_funds_list[index]
		strcontent2 = '\'' + all_funds_list[index][0].encode('utf-8') + '\',\n'
		print strcontent
		codeFileobject.write(strcontent2)

		if index >= gettopnum:
			break;

	codeFileobject.close()
